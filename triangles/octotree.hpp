		...
		void sift_tree(node_t &cur_root)
		{
			...
			while(it != cur_root.list_of_triangles_.end())
			{
				...
				if(!cur_root.child_[chapter]){
					...
					{
						if((chapter >> j) & 1){
							cur_root.child_[chapter]->left_border_.set_coord(mid.get_coord(j), j);
							cur_root.child_[chapter]->right_border_.set_coord(right.get_coord(j), j);
						}
						else{
							cur_root.child_[chapter]->left_border_.set_coord(left.get_coord(j), j);
							cur_root.child_[chapter]->right_border_.set_coord(mid.get_coord(j), j);
						}
					}
				}
				ListIt tmp = it;
